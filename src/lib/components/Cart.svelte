...
{#if $cartOpen}
    ...
    <div transition:fly={{ x: "100%" }} class="fixed right-0 top-0 w-[350px] bg-white border-l h-full">
        <div class="flex justify-between bg-black text-white p-4 uppercase font-bold">  
            Cart
            <button class="uppercase" on:click={() => $cartOpen = false}><iconify-icon icon="maki:cross" class="text-20"></iconify-icon></button>
        </div>
